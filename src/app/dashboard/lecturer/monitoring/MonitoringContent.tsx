    ...
    <div className="monitoring-container animate-fade-in">
      ...
      <section className="glass-card monitoring-card">
        <div className="card-header flex-between">
          <h3>Nhật ký vi phạm</h3>
          <span className="log-count">{logs.length} bản ghi</span>
        </div>

        {loading ? (
          <p className="text-center text-muted p-8">Đang kết nối hệ thống...</p>
        ) : logs.length === 0 ? (
      ...
      <style jsx>{`
        ...
        .page-header { margin-bottom: 3rem; align-items: flex-start; }
        .back-link { color: var(--primary); text-decoration: none; margin-bottom: 1rem; display: inline-block; }
        
        .status-indicator { display: flex; align-items: center; gap: 0.5rem; background: rgba(34, 197, 94, 0.1); padding: 0.5rem 1rem; border-radius: 20px; border: 1px solid rgba(34, 197, 94, 0.2); }
        ...
        .monitoring-card { padding: 1.5rem; }
        .card-header { margin-bottom: 1.5rem; border-bottom: 1px solid var(--border); padding-bottom: 1rem; align-items: center; }
